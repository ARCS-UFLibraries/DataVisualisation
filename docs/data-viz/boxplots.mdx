...
A boxplot helps answer these questions in a compact visual form. It is based on the five-number summary and is widely used in exploratory data analysis.[web:68]
...
Points that fall far outside the typical range may be shown individually as outliers. Boxplots often use the 1.5 × IQR rule to determine which points count as outliers.[web:68]
...
You can create a basic boxplot in Matplotlib with `plt.boxplot()`.[web:61]
...
Both boxplots and histograms are used to study distributions, but they do so differently.[web:67]
  ...
  items={[
    {
      ...
      href: 'https://colab.research.google.com/github/ARCS-UFLibraries/Coding-Notebooks/blob/main/data-viz/boxplots_lesson.ipynb',
      label: 'Notebook',
    },
  ]}
/>
