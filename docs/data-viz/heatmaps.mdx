...
Now the heatmap is much more informative because the axes tell us what each row and column mean.
...
- scores increase from Test 1 to Test 3 across all subjects,
...
df = pd.DataFrame({
    "math_score":,[14][15][6][10][5]
    "reading_score":,[16][17][7][9][8]
    "writing_score":[18][19][20][21][22]
})
...
A diverging palette such as `"coolwarm"` is often useful when the data has a meaningful center, such as correlation values ranging from negative to positive. Seaborn allows heatmap customization through arguments such as annotation, tick labeling, masking, and other formatting options. 
  ...
  items={[
    {
      ...
      href: 'https://colab.research.google.com/github/YOUR-USERNAME/YOUR-REPO/blob/main/notebooks/heatmaps.ipynb',
      label: 'Notebook',
    },
  ]}
/>
