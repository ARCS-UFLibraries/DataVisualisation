...
Sorting the dataset lets you answer these questions quickly, without manually searching through rows. Sorting helps reveal patterns, rankings, and extreme values that are important in analysis and reporting.
...
This is useful when you want a stable group ordering but still highlight top performers inside each group.
...
These examples show how sorting and `head()` can be combined to find top or bottom performers.

### Google Colab Activity

Using the Students Performance dataset, sort the DataFrame by:

- mathematics score  
- reading score  
- writing score  

For each sort, display the top five students.
